...
You can link to the folder listing by referencing its name, plus a trailing slash, like this: `[advanced/](advanced/)` (results in [advanced/](advanced/)).

By default, Quartz will title the page `Folder: <folder name>` and no description. You can override this by creating an `index.md` file in the folder with the `title` [](authoring%20content.md#Syntax|frontmatter) field. Any content you write in this file will also be used in the folder description.
...
You can link to the tag listing by referencing its name with a `tag/` prefix, like this: `[plugin](tags/plugin.md)` (results in [plugin](tags/plugin.md)).
...
Quartz allows you to define a custom sort ordering for content on both page types. The folder listings are a functionality of the [FolderPage](FolderPage.md) plugin, the tag listings of the [TagPage](TagPage.md) plugin. See the plugin pages for customization options.
