...
| 开发会话 | 多个持久tmux会话 | 断网后仍保留；刚登录时用 `ts` 搜索并回到上次工作现场。 |
...
它会列出仍在运行的tmux会话；输入名称的一部分搜索，回车进入。这样不必记住上次正在使用哪个项目或会话。`tmux`继续负责让任务在断线后存活；`ts`只是更容易找到它们的入口。
...
手机或平板断网时，**不要着急重开 Codex**。重新SSH后再次运行 `ts`，选择仍在运行的会话即可。
...
9. 配置tmux、fzf和会话选择器；按项目或任务创建有意义名称的持久tmux会话，再登录Codex。
...
tmux 是服务器里的“可持续终端窗口”。它留在服务器上运行，所以你回来重新连接后还能接上同一个工作现场。平板环境额外使用一个轻量会话选择器来搜索现有tmux会话；它不替代tmux，也不负责目录管理。
